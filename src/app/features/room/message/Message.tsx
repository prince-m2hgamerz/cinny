...
>(({ room, mEvent, onClose, ...props }, ref) => {
  ...
  const senderDisplayName =
    ...
    senderId ||
    'Unknown';
  const messageContent = mEvent.getContent<{ body?: string }>();
  const excerpt = typeof messageContent?.body === 'string' ? messageContent.body.trim() : undefined;

  const handleSubmit = useCallback(
    async ({ reason, details }: { reason: string; details?: string }) => {
      if (!eventId) throw new Error('Message event not found');
      ...
      await sendReport({
        ...
        reporterDisplayName:
          ...
          getMxIdLocalPart(mx.getSafeUserId() ?? ''),
        targetId: eventId,
        targetName: `Message from ${senderDisplayName}`,
        targetUserId: senderId ?? undefined,
      ...
      await mx.reportEvent(room.roomId, eventId, -100, `${reason}${details ? ` | ${details}` : ''}`);
    },
    [eventId, excerpt, mx, room, senderDisplayName, senderId]
  );
      ...
      <Overlay open={open} backdrop={<OverlayBackdrop />}>
        <OverlayCenter>
          <ReportDialog
            ...
            targetLabel="Message"
            helperText="Report this message for review. The report will be forwarded to the moderation channel."
            requestClose={handleClose}
            onSubmit={handleSubmit}
          />
        </OverlayCenter>
